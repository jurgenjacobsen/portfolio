...
export default {
    ...
    methods: {
        ...
        getPlayerStyles(): { width: string | undefined } | undefined {
            ...
            const timeTotal = this.presence.timestamps.end - this.presence.timestamps.start;
...
<template>
     <div class="flex" v-if="presence?.name === 'Spotify' || showAnyway">
            ...
            <div class="text-box w-full pl-3 pr-1.5 mt-1 grid grid-cols-1 align-center">
                ...
                <span class="md:hidden">{{ truncateString(presence?.details, 28) ?? 'Nothing right now' }}</span>
                <span class="hidden md:block">{{ truncateString(presence?.details, 50) ?? 'Nothing right now' }}</span>
                ...
                <div>
                    ...
                    <span :class="{ 'invisible': !presence?.assets?.large_text  }" class="inline-block ml-1">
                        <span class="md:hidden"><span class="text-gray-500">on</span> {{ truncateString(presence?.assets?.large_text, 28) }}</span>
                        <span class="hidden md:inline-block"><span class="text-gray-500">on</span> {{ truncateString(presence?.assets?.large_text, 50) }}</span>
                    </span>
                </div>
